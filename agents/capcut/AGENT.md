...
이 에이전트는 **vibecut-auto-edit 스킬**을 내장하고 있다. 전체 파이프라인 실행 시:

```python
# 런타임에 스킬 호출
Skill("vibecut-auto-edit")
```

- **전체 파이프라인**(Whisper 전사 → NG 감지 → 구간 생성 → CapCut 적용): 스킬 호출
...
검증 에이전트: `agents/subtitle-verifier/AGENT.md`
